...
if (isset($_GET['act'])) {
    if ($_GET['act'] == 'excel') {
        ...
        header("Content-Disposition: attachment; filename=สรุปรายงานการกู้ยืม(แบบมีคู่).xls");
...
<body>
    ...
    <div class="table-responsive" id="student_table">
        <p>
            <a href="?act=excel" class="btn btn-primary"> ดาวน์โหลดไฟล์สรุปรายงานการกู้ยืม (แบบมีคู่)</a>
        </p>
        <table>
            ...
            <tr>
                ...
                <td style="width: 150px; text-align: center;"><?php echo $row2["IdentificationNumber"]; ?> </td>
                ...
                <td><?php echo number_format($selectBank["tuitionfee"]); ?> </td>
                <td><?php
                        $row2["TuitionFeeAmount"] = str_replace(",", "", $row2["TuitionFeeAmount"]);
                        echo floatval($row2["TuitionFeeAmount"]) - floatval($selectBank["tuitionfee"]); ?> </td>
                <?php
                    }
                }
