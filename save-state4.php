...
$CostOfLiving = $_POST['Cost_Of_Living'];
$status = $_POST['status'];
...
$select = "SELECT * FROM state4 WHERE Student_ID = '$student_id' AND term = '$term' AND year = '$year'";
...
if ($count == 0) {
    $insert = "INSERT INTO state4 (Student_ID, term, year, FeeMoney, Cost_Of_Living, Status)
            VALUES ('$student_id','$term', '$year', '$FeeMoney', '$CostOfLiving', '$status')";
    ...
    $update = "UPDATE state4 SET term = '$term', year ='$year', FeeMoney = '$FeeMoney', Cost_Of_Living='$CostOfLiving', Status='$status'
